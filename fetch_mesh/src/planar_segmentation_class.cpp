...
// TODO:
// finish setting up /start_segmentation and /odom subscribers with the class
// write main method! should take 5 seconds now
...
class PlanarSegmentation {
    ...
    bool start_segmentation;
    geometry_msgs::Pose2D current_pose;
    float transform_x, transform_y;

    static void project_onto_plane(pcl::ModelCoefficients plane, pcl::ModelCoefficients line,
    ...
    static std::vector<float> intersect(pcl::ModelCoefficients l1, pcl::ModelCoefficients l2) {
    ...
    static geometry_msgs::Point toROSPoint(std::vector<float> input) {
    ...
    static bool close_to_cloud(std::vector<float> point, pcl::PointCloud<pcl::PointXYZ> &cloud) {
    ...
    public:
    explicit PlanarSegmentation(ros::NodeHandle *nh, tf::TransformListener *l);
    ...
    void cloud_cb(const sensor_msgs::PointCloud2ConstPtr& cloud_msg) {
        ...
        // transform cloud from the /head_camera_rgb_optical_frame to /base_link or /world with a tf listener
        ...
        while (PlanarSegmentation::planar_seg(*downsampled_cloud)) {
    ...
    bool planar_seg (pcl::PointCloud<pcl::PointXYZ> &cloud) {
        ...
        *cloudptr = cloud;
        ...
        if (not PlanarSegmentation::publish_fitted_rectangle(*cloud_plane, coefficients_plane)) {
            return false;
        }
    ...
    bool publish_fitted_rectangle(pcl::PointCloud<pcl::PointXYZ> &cloud_plane,
        ...
        *cloudptr = cloud_plane;
        ...
        PlanarSegmentation::export_mesh(intersection_points);
...
PlanarSegmentation::PlanarSegmentation(ros::NodeHandle *nh, tf::TransformListener *l) {
    this->start_segmentation = false;
    this->start_sub = nh->subscribe ("/start_segmentation", 5, &PlanarSegmentation::start_cb, this); // this is not working yet
    this->odom_sub = nh->subscribe ("/odom", 5, &PlanarSegmentation::odom_cb, this); // this is not working yet
    this->mesh_sub = nh->subscribe ("/head_camera/depth_registered/points", 5, &PlanarSegmentation::cloud_cb, this); // this is not working yet
    this->mesh_pub = nh->advertise<shape_msgs::Mesh>("fetch_mesh", 5);
    this->transform_x = 0;
    this->transform_y = 0;
    this->tf_listener = l;
}
...
int main (int argc, char **argv) {
    ros::init (argc, argv, "mesh_publisher_class");
    ros::NodeHandle nh;
    tf::TransformListener *tfl;
    PlanarSegmentation pc = PlanarSegmentation(&nh, tfl);
    ros::spin();
}